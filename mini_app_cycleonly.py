...
def predict_crack():
    try:
        ...
        log_message(f"[DEBUG] Raw input: cycle={cycle}")

        cycle = np.clip(cycle, x_min[0], x_max[0])
        log_message(f"[DEBUG] Clamped input: cycle={cycle}")

        input_data = np.array([[cycle]])
        ...
        if pred_mm >= CRITICAL_LENGTH:
            ...
            for future_cycle in future_cycles:
                sim_input = np.array([[future_cycle]])
